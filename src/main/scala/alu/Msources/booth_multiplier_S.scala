...
class booth_multiplier_S(N:Int) extends Module{
    ...
    val bs = Seq.fill(N)(Module(new booth_mult_substep_S(N)))
...
object boothMultiplierS extends App {
  println("Generating the booth multiplier hardware")
  (new chisel3.stage.ChiselStage).emitVerilog(new booth_multiplier_S(64), Array("--target-dir","verilog/"))
