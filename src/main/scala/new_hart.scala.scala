...
class singleCycleHart extends Module {
    ...
    val wirteBackData = MuxLookup(io.resport_instr, 0.U, Array(
        "b?????????????????????????0110111".U -> immediate, // lui
        "b?????????????????????????0010111".U -> immediate + pc, // auipc
        "b?????????????????????????1101111".U -> pc + 4.U, // jal
        "b?????????????????????????1100111".U -> pc + 4.U, //jalr
